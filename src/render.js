  ...
  return `
    <div class="ba${isReal(first) ? " is-real" : ""}" data-ba>
      <figure class="ba__stage">
        <img class="ba__img ba__img--after" src="/assets/img/${afterOf(first)}.jpg" alt="After: the finished result" data-ph="${esc(first.label)} — after" width="1040" height="650" decoding="async">
        <div class="ba__before" data-ba-before>
          <img class="ba__img ba__img--before" src="/assets/img/${beforeOf(first)}.jpg" alt="Before: the same roof beforehand" data-ph="${esc(first.label)} — before" width="1040" height="650" decoding="async" aria-hidden="true">
        </div>
...
const areaFooter = (site) => site.serviceAreas.map((a) => `<li><a href="#estimate">Roofing in ${esc(a)}</a></li>`).join("");
...
function jsonLd(site) {
  const business = {
    ...
    image: `${site.seo.url}/assets/img/roof.jpg`,
    "@id": site.seo.url,
    url: site.seo.url,
